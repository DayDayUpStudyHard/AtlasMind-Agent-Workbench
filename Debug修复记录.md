...
推送 `2073465` 后，GitHub Actions 的 Java 任务以退出码 `126` 失败，Python AI 任务以退出码 `2` 失败，导致 Docker 验证被依赖条件跳过。前端两个生产构建正常。

### 根因与修复

1. `agent-server/mvnw` 在 Git 中的文件模式为 `100644`，Ubuntu Runner 无法执行 `./mvnw`。已将其模式改为 `100755`。
2. Python 测试会导入基于 LangGraph 的 Agent Runtime，但 `requirements.txt` 未声明 LangGraph 依赖；本地已安装的依赖掩盖了该问题，GitHub 的干净环境在测试收集阶段退出。主依赖现通过 `-r requirements-graph.txt` 显式安装 LangGraph 及 checkpoint 依赖。

### 验证结果

- `agent-server\\mvnw.cmd test -B`：43 项通过。
- `tools/chat-assistant/backend/python -m pytest`：387 项通过。
- `python -m pip install --dry-run -r requirements.txt`：确认包含 `langgraph` 与 `langgraph-checkpoint`。
