...
### 2.4 评分器修复后的重算口径补记（2026-08-14 晚）

§2.1/§2.2 的召回数字是**官方冻结口径**（评分器修复前，历史值保留在 DB `summary_json.officialHighRiskRecall`）。当晚评分器维度门禁缺陷修复（a95f5cf：domainKey 优先 + 维度桶映射 + 强文本旁路，见 phase3 报告 §3.5）后，对全部存量 run 做了重算（只 UPDATE 存量行，未重跑）：

| Run | 引擎 | 官方召回 | 重算召回 | 说明 |
|---|---|---|---|---|
| 25 | langgraph | 61.67% | **93.33%** | 10 例修正（见 phase3 报告 §3.1） |
| 22 | langgraph | 64.44% | **90.0%** | 10 例修正 |
| 18 | langgraph | 58.33% | **80.0%** | 9 例修正，其中 CR-030 因旧 artifact 词表漂移 1.0→0.0 |
| 19 | langgraph | 58.33% | **78.33%** | 8 例修正 |
| 15 | langgraph | 57.78% | **83.33%** | 9 例修正 |
| 16 | langgraph | 58.33% | **80.0%** | 9 例修正，其中 CR-023 因旧 artifact 词表漂移 1.0→0.0 |
| 26 | legacy | 35.0% | **45.0%** | 4 例修正 |
| 20 | legacy | 31.67% | **41.67%** | 5 例修正 |
| 14 | legacy | 1.67% | **11.67%** | 4 例修正（19 例无 artifact 不计分） |
| 13 | legacy | 1.67% | **3.33%** | 1 例修正（18 例无 artifact 不计分） |
| 17 | legacy | 1.67% | 1.67% | 无变化（12 例可评均未误杀） |
| 9 | legacy | 0.0% | — | 30 例 artifact 均无 findings（早期格式），不可重算 |

重算口径下两条历史结论修正：

- **§2.1「run 25 比历史最佳 run 22 低 2.7pp」反转**：重算后 run 25（93.33%）反而高于 run 22（90.0%），官方低分是评分器误杀伪像，非真实下降。
- run 16/18 各有 1 例（CR-023/CR-030）重算后从 1.0 降到 0.0：早期 v1 artifact 的 domainKey 用旧词表（如 `liability_remedies`），与当前维度桶映射不对齐——引擎侧词表演进所致，非评分器回归（run 25 同 case 重算均为 1.0）。
- 不可重算的 run：要素/日程/核验任务（23/24/27/28/29，评分器不同）、ds=13 压力测试（run 10，artifact 全为 infraFailed 空壳）、runs 33/34（golden，重算无变化）、run 9（见上表）。unscoreable case（artifact 无 findings / 期望侧无 HIGH）保留存量值计入均值，分母与官方口径一致。
