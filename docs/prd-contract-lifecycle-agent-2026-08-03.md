...
3. 用户选择企业批准模板，系统生成受控初稿，或上传对方合同正文。
4. 系统完成解析并展示主体、金额、期限和条款目录。
5. 用户点击”开始合同审查”。
6. 前端实时展示 Planner、知识检索、规则检查和 Reflection。
7. **Agent 发现**：付款条件偏离公司制度、验收标准缺失、自动续签通知期过短。
8. 用户点击风险，分别跳转到合同原文和企业制度条款（双引用）。
9. Agent 提出补材料、修改条款和法务复核动作。
...
该演示完整覆盖 Agent 的五种工作模式：**被动审查、主动建议、自主巡检、异常分析、自我改进**。核心差异化不在”能不能审查合同”，而在”无人指令时，Agent 在做什么”。
